...
int main() {
    int desiredLength = 10000; // Задаем желаемую длину строки
    string X = generateRandomUpperCaseString(desiredLength);
    string Y = generateRandomUpperCaseString(desiredLength);
    
    

    cout << "НОП (" << "X" << ", " << "Y" << ") методом полного перебора: " << lcs_brute_force(X, Y) << endl;
    cout << "НОП (" << "X" << ", " << "Y" << ") методом динамического программирования: " << lcs_dynamic_programming(X, Y) << endl;
